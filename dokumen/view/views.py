...
from ..form.form import DokumenForm, EditForm
...
class dashboard(generic.TemplateView):
	...
	def get_context_data(self, **kwargs):
		...
		if self.request.user.is_admin:
			context['data_surat_masuk'] = Dokumen.objects.all()
			# context['data_surat_masuk'] = Dokumen.objects.exclude(status=3)
			context['data_surat_keluar'] = Dokumen.objects.all()
			...
			# sebelumnya ada exclude status = 3
			context['data_surat_keluar'] = Dokumen.objects.filter(
				fungsi=self.request.session['KODE_FUNGSI']).order_by('-pk')
			context['data_surat_masuk'] = Dokumen.objects.filter(
				tujuan=self.request.session['KODE_FUNGSI']).values('nomor_surat_lengkap','klasifikasi__kode','klasifikasi__nama_klasifikasi','pejabat_penandatangan','tujuandokumen__status','slug').order_by('-pk')
			# context['coba'] = data
...
class BuatDokumen(generic.edit.CreateView):
	...
	def get_form_kwargs(self):
		...
		if self.request.user.is_admin:
			...
			# print(ProfileUser.objects.get(user=self.request.user.pk))
			# print(self.request.session['KODE_FUNGSI'])
			kwargs.update({'fungsi': self.request.session['KODE_FUNGSI']})
		return kwargs
...
class BacaDokumen(generic.View):
	def get(self, request):
		...
		# print(fungsi)
		slug = request.GET.get('slug', None)
		a = TujuanDokumen.objects.get(dokumen__slug=slug, fungsi=self.request.session['KODE_FUNGSI'])
		a.status = 1
		a.save()
		data = {
			'read': True
	...
	def get_context_data(self, **kwargs):
		...
		if self.request.user.is_admin:
			...
			context['data_nota_dinas'] = Dokumen.objects.filter(
				fungsi=self.request.session['KODE_FUNGSI'])
		# context['jenis_dokumen'] = JenisDokumen.objects.all()
		return context

# class print(generic.DetailView):
# 	template_name = 'laporan\laporan-nota-dinas.html'
# 	model = Dokumen
