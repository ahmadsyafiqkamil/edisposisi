    ...
    if request.method == 'POST':
        if request.POST['ps1'] == request.POST['ps2']:
            try:
                ...
                user = User.objects.create_user(email=request.POST['email'],password=request.POST['ps1'],user_name=request.POST['username'],user_nama_lengkap=request.POST['fullname'])
                auth.login(request,user)
